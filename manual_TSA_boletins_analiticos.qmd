...
A receção de um boletim retificado (com número de relatório e ID de colheita igual) espoleta a deteção de duplicação pelo sistema e a atualização do registo existente na lista de resultados com os novos valores. O PDF original não é substituído, sendo conservado no arquivo.
...
| Corpo do e-mail: | Identificação do programa (ex.: PVSA, PSAC, etc.); número do boletim (`n_boletim`), se aplicável; descrição sucinta da anomalia (ex.: valor extraído pelo sistema vs. valor real no boletim). |
